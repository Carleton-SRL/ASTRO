function [lambda,ecltyp] = Shadow(pccor,ccor,pscor,sbcor,bcor,sbpcor,ConstantParameters)
%--------------------------------------------------------------------------
...
if(rb<=rc)
    ...
    rs=ConstantParameters.R_Sun/rb;
    rp=ConstantParameters.R_Earth/rsbx;
    sep=sqrt(sepp(1)^2+sepp(2)^2+sepp(3)^2)/rsbx;
    
    lambda = dynamics.get_lambda(rs,rp,sep);
end
...
if(lambda<1)
    ...
    rs=ConstantParameters.R_Sun/rb;
    rp=ConstantParameters.R_Moon/rsbx;
